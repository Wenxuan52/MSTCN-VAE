...
class EncoderMSTCN(nn.Module):
    def __init__(self, C, num_output, num_person=1, in_channels=3, num_point=22, T=100, classifeature=10):
...
class DecoderLinear(nn.Module):
    def __init__(self, in_channels=3, num_point=22, classifeature=100, T=100):
        super(DecoderLinear, self).__init__()
        self.in_channels = in_channels
        self.num_point = num_point
        self.T = T
        self.CV = in_channels * num_point
        self.fcn1 = nn.Linear(classifeature, in_channels * num_point)
        self.fcn2 = nn.Linear(in_channels * num_point, in_channels * num_point * T)

    def forward(self, x):
        x = self.fcn1(x)
        deout = self.fcn2(x)
        N, _ = deout.size()
        deout = deout.view(N, self.T, self.CV)
        return deout


class MSTCN_VAE(nn.Module):
    def __init__(self, C, num_output, num_person=1, in_channels=3,
                 num_point=22, classifeature=100, fix_state=False, fix_weight=False):
        ...
        self.decoder = DecoderLinear(classifeature=classifeature).to(device)
...
if __name__ == "__main__":
    ...
    mtv = MSTCN_VAE(3, 96, classifeature=100)
